...
public class EventListener extends ListenerAdapter {
  ...
  public void onMessageReceived(MessageReceivedEvent event) {
      ...
      if (!event.getAuthor().getId().equals(HifumiBot.getSelf().getJDA().getSelfUser().getId())) {
        Messaging.logInfo(
            ...
            "DM sent to Hifumi by user "
                ...
                + "\n"
                + "```\n"
                + "Message content displayed raw format, truncated to 500 chars. Original length: "
    ...
    if (HifumiBot.getSelf()
        ...
        .hasPermission(PermissionLevel.GUEST, event.getMember())) {
    ...
    if (!HifumiBot.getSelf()
      ...
      if (Messaging.hasBotPing(event.getMessage())) {
        Messaging.sendMessage(
            event.getChannel(), "You are pinging a bot.", event.getMessage(), false);
      }
    }
    ...
    if (event.getMember() != null && event.getMember().getRoles().isEmpty()) {
      Instant joinTime =
          event
              .getMember()
              .getGuild()
              .retrieveMemberById(event.getAuthor().getId())
              .complete()
              .getTimeJoined()
              .toInstant();

      if (Duration.between(joinTime, now).toSeconds()
          >= HifumiBot.getSelf().getConfig().roles.autoAssignMemberTimeSeconds) {
        event
            .getGuild()
            .addRoleToMember(
                event.getMember(),
                event
                    .getGuild()
                    .getRoleById(HifumiBot.getSelf().getConfig().roles.autoAssignMemberRoleId))
            .complete();
      }
    }
    ...
    for (Role role : event.getRoles()) {
      if (role.getId().equals(HifumiBot.getSelf().getConfig().roles.warezRoleId)
          && !HifumiBot.getSelf()
              .getWarezTracking()
              .warezUsers
              .containsKey(event.getUser().getId())) {
        HifumiBot.getSelf()
            .getWarezTracking()
            .warezUsers
            .put(event.getUser().getId(), OffsetDateTime.now());
  ...
  public void onGuildMemberJoin(GuildMemberJoinEvent event) {
    ...
    if (HifumiBot.getSelf().getConfig().enableBotKicker) {
      if (cooldown != null && OffsetDateTime.now().isAfter(cooldown)) {
        ...
        Messaging.logInfo(
            "EventListener",
            "onGuildMemberJoin",
            "Cooldown has been released, new users are now able to join the server again.");
      }
      ...
      if (joinEvents.size() == 3) {
        ...
        if (Math.abs(Duration.between(minTime, maxTime).toSeconds()) < 1) {
          ...
          Messaging.logInfo(
              "EventListener",
              "onGuildMemberJoin",
              "Cooldown has been tripped by three users joining in under one second; new users will"
                  + " be automatically messaged and kicked for the next five minutes.");
    ...
    if (HifumiBot.getSelf().getWarezTracking().warezUsers.containsKey(event.getUser().getId())) {
      ...
      Messaging.sendMessageEmbed(
          event
              .getGuild()
              .getTextChannelById(HifumiBot.getSelf().getConfig().channels.systemOutputChannelId),
          eb.build());
    }
  }

  public void waitForMessage(String userId, Message msg) {
    if (messages.containsKey(userId)) messages.get(userId).delete().complete();
  ...
  public void finalizeMessage(Message msg, String gameName, String userId) {
    ...
    for (String knownIssue : wikiPage.getKnownIssues()) issueList.append(knownIssue).append("\n");
    ...
    for (String fixedIssue : wikiPage.getFixedIssues()) fixedList.append(fixedIssue).append("\n");
