    ...
    try {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        if (line.isBlank() || line.startsWith("//")) {
          ...
          if (lineStart.equals("author")
              || lineStart.equals("comment")
              || lineStart.equals("gametitle")) {
            continue;
          } else if (lineStart.equalsIgnoreCase("author")
              || lineStart.equalsIgnoreCase("comment")
              ...
              try {
                ...
                if (params.length == 5) {
                  ...
                  if (params[1].equals("EE") || params[1].equals("IOP")) {
                    // Do nothing
                  } else if (params[1].equalsIgnoreCase("EE")
                      || params[1].equalsIgnoreCase("IOP")) {
                  ...
                  if (params[3].equals("byte")
                      || params[3].equals("short")
                      || params[3].equals("word")
                      || params[3].equals("double")
                      || params[3].equals("extended")) {
                    // do nothing
                  } else if (params[3].equalsIgnoreCase("byte")
                      || params[3].equalsIgnoreCase("short")
      ...
      bodyBuilder
          ...
          .append(
              "============================== Pnach Parse Results =============================")
          .append("\n")
          .append("(*) = Information (!) = Warning (X) = Critical")
          .append("\n\n");
      ...
      if (hasLines) {
        ...
        if (bodyBuilder.toString().getBytes().length
          ...
          Messaging.sendMessage(
              ...
              "Pnach_" + message.getAuthor().getName() + ".txt",
              bodyBuilder.toString());
        } else {
          Messaging.sendMessage(
              message.getChannel(),
              ":warning: Your pnach generated such a large results file that I can't upload it. A"
                  + " human is gonna have to read through your pnach manually.");
        }
      } else {
        Messaging.sendMessage(
            message.getChannel(), ":white_check_mark: All good, nothing to report!");
      }
    } catch (IOException e) {
